...
class TestChronograph(object):
    def test_view_job_not_add(self, log_for_test, db, chronograph, auction):
    ...
    def test_view_job_add(self, log_for_test, db, chronograph, auction):
        ...
        log_for_test.info('+++++ assertion passed')
    ...
    def test_listing(self, log_for_test, db, chronograph, auction):
        log_for_test.info('   START   TEST')
        auction.prepare_auction_document()
        db.view('chronograph/start_date')

        log_for_test.info('+++++ chronograph starts running')
        chronograph.join(0.1)

        log_for_test.info('+++++ check if job_is_added')
        log_for_test.info(job_is_not_active())
        assert job_is_added()
        assert job_is_not_active()

        log_for_test.info('+++++ check if job_is_active')
        chronograph.join(3.4)
        log_for_test.info('+++++ pause passed')
        assert job_is_not_added()
        assert job_is_active()
        log_for_test.info('+++++ assertion passed')
